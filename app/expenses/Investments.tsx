...
export default function InvestmentsExpense() {
  ...
  const filteredSuggestions = investment
    ? popularDescriptions.filter(opt => opt.toLowerCase().includes(investment.toLowerCase()))
    : popularDescriptions;
  ...
  return (
    ...
    >
      <View style={{ flex: 1, justifyContent: "center", alignItems: "center", paddingHorizontal: 24 }}>
        ...
        <TouchableOpacity
          style={{ backgroundColor: investment && investmentType ? '#FFA726' : '#FFE0B2', borderRadius: 8, padding: 14, width: '100%', alignItems: 'center' }}
          disabled={!investment || !investmentType}
          onPress={handleSubmit}
        >
          <Text style={{ color: investment && investmentType ? '#fff' : '#aaa', fontSize: 16, fontWeight: 'bold' }}>Submit</Text>
        </TouchableOpacity>
      </View>
    </LinearGradient>
  );
